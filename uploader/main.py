...
class NackReceivedException(Exception):
    ...
    def __init__(self, sequence, error_code):
        ...
        self.description = self.ERROR_DESCRIPTIONS.get(
            error_code, f"Unknown error code: {error_code}"
        )
        super().__init__(
            f"NACK received for sequence {sequence}, error code: {error_code} - {self.description}"
        )
...
class UploaderThread(QThread):
    ...
    def __init__(
        self, patched_file_path, receiver_thread, port="COM6", baudrate=115200
    ):
        super().__init__()
            ...
            if packet:
                ...
                if packet.service_type == PacketType.ACK:
                    if packet.sequence_count == expected_sequence:
                        ...
                        self.uart_data.emit(
                            f"[Warning: ACK seq mismatch: expected {expected_sequence}, got {packet.sequence_count}]"
                        )
                    ...
                    self.uart_data.emit(
                        f"[Unexpected packet type: {packet.service_type}]"
                    )
    ...
    def run(self):
        ...
        try:
            ...
            if not ser or not ser.is_open:
                self.finished.emit(
                    False, f"Serial port not open. Please connect to the device first."
                )
                return
            ...
            print(
                f"Sending command - Header: {cmd_header.hex()}, Data: {cmd_data.hex()}"
            )
            ...
            if not self.wait_for_ack_packet(0):
                self.finished.emit(
                    False,
                    "Board did not acknowledge upload command (timeout or system not configured)",
                )
                return
            ...
            print(
                f"START_UPLOAD - Header: {start_header.hex()}, Data: {start_data.hex()}"
            )
            ...
            for attempt in range(self.max_retries):
                ...
                self.status.emit(
                    f"Sending START packet... Attempt {attempt + 1}/{self.max_retries}"
                )
                # Send header first
            ...
            if not start_sent:
                self.finished.emit(
                    False, "Failed to send START packet after multiple retries"
                )
                return
            ...
            for i in range(0, total_size, chunk_size):
                ...
                for attempt in range(self.max_retries):
                    ...
                    if attempt > 0:
                        self.status.emit(
                            f"Retrying chunk {chunk_num}/{total_chunks}... Attempt {attempt + 1}/{self.max_retries}"
                        )
                    ...
                    if self.wait_for_ack_packet(sequence):
                        ...
                        self.status.emit(
                            f"Timeout waiting for ACK (attempt {attempt + 1})"
                        )
                        if attempt < self.max_retries - 1:
                            time.sleep(0.3)
                ...
                if not chunk_sent:
                    self.finished.emit(
                        False,
                        f"Failed to send chunk {chunk_num} after {self.max_retries} retries",
                    )
                    return
                ...
                self.status.emit(
                    f"Uploading: {bytes_sent}/{total_size} bytes ({progress_percent}%) - Chunk {chunk_num}/{total_chunks}"
                )
            ...
            self.finished.emit(
                True, f"Successfully uploaded {total_size} bytes using ECSS protocol!"
            )
            ...
            while (time.time() - start_time) < self.ack_timeout:
                ...
                if pkt:
                    if pkt.service_type == PacketType.ACK:
                        self.finished.emit(
                            True, f"ACK received (seq {pkt.sequence_count})"
                        )
                        return
                    elif pkt.service_type == PacketType.NACK:
                        error_code = pkt.data[0] if len(pkt.data) > 0 else 0
                        desc = NackReceivedException.ERROR_DESCRIPTIONS.get(
                            error_code, f"Unknown error code: {error_code}"
                        )
                        self.finished.emit(
                            False, f"NACK received - error {error_code}: {desc}"
                        )
                        return
                    else:
                        self.finished.emit(
                            False, f"Unexpected packet type: {pkt.service_type}"
                        )
                        return
...
# ---------------------------------------------------------------------------
# BSW boot-event report parsing
# ---------------------------------------------------------------------------
_BSW_REPORT_MAGIC = 0xBEEF0042
_BSW_REPORT_COUNT = 5
# Struct layout mirrors bsw_report_t (packed, little-endian ARM):
#   uint32 magic | uint8 type | uint8 outcome | uint8 primary_slot | uint8 pad
#   uint32 rollback_counter | uint16 primary_version | uint16 secondary_version
#   uint32 step_flags
_REPORT_STRUCT = struct.Struct("<IBBBBIHHI")  # 20 bytes

_REPORT_TYPE_NAMES = {0x01: "NOMINAL", 0x02: "UPDATE", 0x03: "SWAP"}
...
def _format_reports(raw_list: list[bytes]) -> str:
    """Parse and pretty-print a list of raw bsw_report_t byte payloads."""
    lines = []
    for age, raw in enumerate(raw_list):
        age_label = (
            "newest"
            if age == 0
            else ("oldest" if age == _BSW_REPORT_COUNT - 1 else f"{age} ago")
        )
        lines.append("=" * 62)
        lines.append(f"  Report {age + 1}/{_BSW_REPORT_COUNT}  [{age_label}]")
        lines.append("=" * 62)

        if len(raw) < _REPORT_STRUCT.size:
            lines.append("  (payload too short — slot may be corrupt)")
            lines.append("")
            continue

        magic, rtype, outcome, pslot, _pad, counter, pver, sver, flags = (
            _REPORT_STRUCT.unpack(raw[: _REPORT_STRUCT.size])
        )

        if magic != _BSW_REPORT_MAGIC:
            lines.append(f"  (empty slot — magic 0x{magic:08X} ≠ 0xBEEF0042)")
            lines.append("")
            continue

        type_name = _REPORT_TYPE_NAMES.get(rtype, f"0x{rtype:02X}")
        slot_name = _SLOT_NAMES.get(pslot, f"slot {pslot}")
        outcome_str = "SUCCESS" if outcome == 0 else f"FAILED at step {outcome}"
        pver_str = str(pver) if pver != 0xFFFF else "n/a"
        sver_str = str(sver) if sver != 0xFFFF else "n/a"

        lines.append(f"  Type              : {type_name}")
        lines.append(f"  Outcome           : {outcome_str}")
        lines.append(f"  Primary slot      : {slot_name}")
        lines.append(f"  Rollback counter  : {counter}")
        lines.append(f"  Primary version   : {pver_str}")
        lines.append(f"  Secondary version : {sver_str}")
        lines.append(f"  Step flags        : 0x{flags:08X}")

        for mask, desc in _FLAGS_BY_TYPE.get(rtype, []):
            tick = "✓" if (flags & mask) else "✗"
...
class ReportFetchThread(QThread):
    """Send command '7', receive 5 BSW_REPORT_DATA packets, emit formatted text."""

    finished = Signal(str)

    def __init__(self, serial_port, receiver_thread):
        super().__init__()
        self.serial_port = serial_port
        self.receiver_thread = receiver_thread
        self.ack_timeout = 3.0
        self.report_timeout = 3.0

    def run(self):
        try:
            ser = self.serial_port
            if not ser or not ser.is_open:
                self.finished.emit("ERROR: Not connected to board!")
                return

            # Send command '7'
            packet = create_command_packet(0, "7")
            ser.write(packet.pack_header())
            ser.flush()
            time.sleep(0.01)
            ...
            while (time.time() - start) < self.ack_timeout:
                ...
                if pkt.service_type == PacketType.NACK:
                    self.finished.emit("ERROR: Board rejected REPORT command (NACK)")
                    return

            if not ack_ok:
                self.finished.emit("ERROR: Timeout waiting for ACK from board")
                return

            # Receive 5 REPORT_DATA packets (newest → oldest)
            raw_reports = []
            for i in range(_BSW_REPORT_COUNT):
                pkt = self.receiver_thread.get_packet(timeout=self.report_timeout)
                if pkt is None:
                    self.finished.emit(
                        f"ERROR: Timeout waiting for report packet {i + 1}/{_BSW_REPORT_COUNT}"
                    )
                    return
                if pkt.service_type != PacketType.REPORT_DATA:
                    self.finished.emit(
                        f"ERROR: Unexpected packet type 0x{pkt.service_type:02X} (expected REPORT_DATA)"
                    )
                    return
                raw_reports.append(pkt.data)

            self.finished.emit(_format_reports(raw_reports))
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.connection_status_label.setStyleSheet(
            "color: #cc0000; font-weight: bold; padding: 5px;"
        )
        ...
        for radio in (
            self.ecdsa_algo_radio,
            self.mldsa_algo_radio,
            self.hybrid_algo_radio,
        ):
        ...
        self.use_existing_keys_radio.toggled.connect(
        ...
        self.cmd_report_btn = QPushButton("7 - Get Reports")
        ...
        self.cmd_report_btn.setMinimumHeight(32)
        self.cmd_report_btn.clicked.connect(self._fetch_reports)

        for btn in [
            self.cmd_boot_btn,
            self.cmd_update_btn,
            self.cmd_swap_btn,
            self.cmd_versions_btn,
            self.cmd_reset_btn,
            self.cmd_skip_btn,
        ]:
        ...
        # Report button on its own row
        report_row = QHBoxLayout()
        report_row.addWidget(self.cmd_report_btn)
        report_row.addStretch()
        cmd_layout.addLayout(report_row)
        ...
        self.asw_cmd_input.setPlaceholderText(
            "1 = OK (set NOMINAL)   2 = FAULT (reset only)"
        )
    ...
    def upload_to_device(self):
        ...
        try:
            ...
            self.uploader_thread = UploaderThread(
                self.patched_file, self.receiver_thread
            )
    ...
    def populate_com_ports(self):
        ...
        if available_ports:
            ...
            if hasattr(self, "log_text"):
                self.log(
                    f"Found {len(available_ports)} COM port(s): {', '.join(available_ports)}"
                )
        else:
    ...
    def on_connection_lost(self):
        ...
        self.connection_status_label.setStyleSheet(
            "color: #cc6600; font-weight: bold; padding: 5px;"
        )
    ...
    def _send_command(self, char):
        ...
        try:
            ...
            self.command_thread = CommandSenderThread(
                self.serial_port, self.receiver_thread, char
            )
    ...
    def _fetch_reports(self):
        """Request the 5 stored boot-event reports from the board."""
        if not self.serial_port or not self.serial_port.is_open:
            self.log("ERROR: Not connected to board!")
            return
        if not self.receiver_thread:
        ...
        self.log("Requesting boot reports from board...")
        for btn in self._cmd_buttons:
            btn.setEnabled(False)

        self.report_thread = ReportFetchThread(self.serial_port, self.receiver_thread)
        self.report_thread.finished.connect(self.on_report_finished)
        self.report_thread.start()

    def on_report_finished(self, text: str):
        """Display parsed boot reports in the log."""
        self.log("─" * 62)
        self.log("Boot Event Reports")
        self.log("─" * 62)
    ...
    def log_uart_data(self, data):
        ...
        self.log_text.append(
            f"<span style='color: #0066cc;'><b>[BOARD]</b> {annotated}</span>"
        )
        # Auto-scroll to bottom
        scrollbar = self.log_text.verticalScrollBar()
        scrollbar.setValue(scrollbar.maximum())
