"""
ecss_packet.py — ECSS-inspired packet protocol for the STM32F439 uploader.

Implements the same wire format as the firmware (NucleoF439-BSP/Inc/ecss_packet.h):

  Wire format (7-byte header + variable payload):
    [0]    version_type_flags  — Version(3) | Type(1) | Reserved(4)
    [1]    service_type        — one of PacketType
...
Public API
----------
  ECSSPacket          — packet class (pack/unpack)
  PacketType          — service-type enum
  create_start_packet — build PKT_START_UPLOAD
  create_data_packet  — build PKT_DATA_CHUNK
  create_end_packet   — build PKT_END_UPLOAD
  create_command_packet — build a single-byte command packet
"""
...
class ECSSPacket:
    ...
    def __init__(
        self,
        service_type: PacketType,
        sequence_count: int,
        data: bytes = b"",
        is_telecommand: bool = True,
    ...
    def unpack_header(cls, header_bytes: bytes) -> "ECSSPacket":
        ...
        if len(header_bytes) != cls.HEADER_SIZE:
            raise ValueError(
                f"Header must be exactly {cls.HEADER_SIZE} bytes, got {len(header_bytes)}"
            )

        # Unpack header
        version_type_flags, service_type, sequence_count, data_length, checksum = (
            struct.unpack(">BBHHB", header_bytes)
        )
        ...
        if checksum != calculated_checksum:
            raise ValueError(
                f"Header checksum mismatch: {checksum} != {calculated_checksum}"
            )
    ...
    def unpack(cls, packet_bytes: bytes) -> "ECSSPacket":
        ...
        if len(packet_bytes) < cls.HEADER_SIZE:
            raise ValueError(
                f"Packet too short: {len(packet_bytes)} < {cls.HEADER_SIZE}"
            )

        # Unpack header
        version_type_flags, service_type, sequence_count, data_length, checksum = (
            struct.unpack(">BBHHB", packet_bytes[: cls.HEADER_SIZE])
        )

        print(
            f"Unpacking packet: version/type/flags={version_type_flags:08b}, "
        ...
        if len(packet_bytes) < data_end:
            raise ValueError(
                f"Packet data incomplete: expected {data_length} bytes, got {len(packet_bytes) - cls.HEADER_SIZE}"
            )
    ...
    def __repr__(self):
        return (
            f"ECSSPacket(service={PacketType(self.service_type).name}, "
