...
static const byte pubKeyDer[] = {
    ...
    0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07, 0x03, 0x42, 0x00, 0x04, 0xa3, 0xed, 0xaa, 0x6c, 0x6e,
    0x70, 0x64, 0x53, 0x34, 0xe2, 0x91, 0xed, 0xe2, 0x3d, 0xe9, 0xef, 0xd2, 0xd6, 0x8b, 0x8d, 0xad,
    0xde, 0x0b, 0xb6, 0x13, 0x7a, 0xf7, 0x77, 0xfe, 0xda, 0x85, 0x67, 0x4f, 0x70, 0x51, 0xb3, 0x9b,
    0x39, 0x02, 0x83, 0x1f, 0xf2, 0x42, 0x5f, 0x19, 0x92, 0x59, 0x37, 0x37, 0x86, 0x6d, 0xb7, 0x95,
    0xb2, 0x2e, 0xac, 0xed, 0x11, 0xb8, 0x42, 0xc8, 0xbd, 0x6a, 0xb0};
...
static int16_t algo_verify(const byte* msg, uint32_t msgLen, const byte* sig, uint32_t sigLen)
{
	...
	int verified = 0;
	int ret = wc_ecc_verify_hash(sig, derLen, digest, WC_SHA256_DIGEST_SIZE, &verified, &eccKey);
	if (ret != 0)
	{
		printf("wc_ecc_verify_hash: %d\r\n", ret);
		return 0;
	}
...
const sig_algo_t g_algo = {.name    = "ECDSA-P256",
                           .init    = algo_init,
                           .verify  = algo_verify,
