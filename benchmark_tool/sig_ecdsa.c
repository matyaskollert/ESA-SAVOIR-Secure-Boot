...
static int ecdsa_generate_keys(sig_ctx_t* ctx,
                               const char* priv_path, const char* pub_path)
{
    ...
    if (io_write_file(priv_path, priv_der, (size_t)priv_len) != 0) return -1;
    if (io_write_file(pub_path,  pub_der,  (size_t)pub_len)  != 0) return -1;
...
static int ecdsa_load_keys(sig_ctx_t* ctx,
                           const char* priv_path, const char* pub_path)
{
    ecdsa_ctx_t* context = (ecdsa_ctx_t*)ctx;
    int result_code;
    ...
    if (priv_path)
    {
        ...
        result_code = wc_EccPrivateKeyDecode(file_buffer, &decode_index,
    ...
    if (pub_path)
    {
        ...
        if (io_read_file(pub_path, &file_buffer, &buffer_length) != 0)
            return -1;
        word32 decode_index = 0;
        result_code = wc_EccPublicKeyDecode(file_buffer, &decode_index,
                                            &context->key, (word32)buffer_length);
        free(file_buffer);
        if (result_code != 0)
        {
...
static int ecdsa_sign(sig_ctx_t* ctx,
                      ...
                      uint8_t* sig_out, size_t* sig_written)
{
    ...
    word32 sig_len = 72;
    result_code = wc_ecc_sign_hash(digest, WC_SHA256_DIGEST_SIZE,
                                   sig_out, &sig_len, &context->rng, &context->key);
    if (result_code != 0)
    {
...
static int ecdsa_verify(sig_ctx_t* ctx,
                        const uint8_t* msg, size_t msg_len,
                        const uint8_t* sig, size_t sig_len)
{
    ...
    if (der_len > (word32)sig_len) der_len = (word32)sig_len;

    int verified = 0;
    int result_code = wc_ecc_verify_hash(sig, der_len,
