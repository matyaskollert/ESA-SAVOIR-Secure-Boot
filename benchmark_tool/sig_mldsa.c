...
static const mldsa_info_t s_info[2] = {
    {"ML-DSA-44", 2, DILITHIUM_LEVEL2_SIG_SIZE, DILITHIUM_LEVEL2_PUB_KEY_SIZE,
     DILITHIUM_LEVEL2_PRV_KEY_SIZE},
    {"ML-DSA-65", 3, DILITHIUM_LEVEL3_SIG_SIZE, DILITHIUM_LEVEL3_PUB_KEY_SIZE,
     DILITHIUM_LEVEL3_PRV_KEY_SIZE},
};
...
static int mldsa_generate_keys(sig_ctx_t* ctx, const char* priv_path, const char* pub_path)
{
	mldsa_ctx_t* context = (mldsa_ctx_t*)ctx;
	int result_code;
	...
	result_code = wc_dilithium_export_key(&context->key, private_key_buffer, &private_key_len,
...
static int mldsa_load_keys(sig_ctx_t* ctx, const char* priv_path, const char* pub_path)
{
	mldsa_ctx_t* context = (mldsa_ctx_t*)ctx;
	int result_code;
	...
	if (priv_path)
	{
		...
		if (io_read_file(priv_path, &file_buffer, &buffer_length) != 0)
			return -1;
		result_code =
		    wc_dilithium_import_private(file_buffer, (word32)buffer_length, &context->key);
		free(file_buffer);
		if (result_code != 0)
		{
	...
	if (pub_path)
	{
		...
		if (io_read_file(pub_path, &file_buffer, &buffer_length) != 0)
			return -1;
		result_code = wc_dilithium_import_public(file_buffer, (word32)buffer_length, &context->key);
		free(file_buffer);
		if (result_code != 0)
		{
...
static int mldsa_sign(sig_ctx_t* ctx, const uint8_t* msg, size_t msg_len, uint8_t* sig_out,
                      size_t* sig_written)
{
	mldsa_ctx_t* context = (mldsa_ctx_t*)ctx;
	int result_code;
	...
	result_code          = wc_dilithium_sign_msg(msg, (word32)msg_len, sig_out, &signature_len,
...
static int mldsa_verify(sig_ctx_t* ctx, const uint8_t* msg, size_t msg_len, const uint8_t* sig,
                        size_t sig_len)
{
	mldsa_ctx_t* context = (mldsa_ctx_t*)ctx;

	int verified    = 0;
	int result_code = wc_dilithium_verify_msg(sig, (word32)sig_len, msg, (word32)msg_len, &verified,
	                                          &context->key);
	if (result_code != 0)
	{
