                        ...
                        uint8_t* out_buf, size_t out_buf_capacity)
{
	...
	size_t sign_len = (size_t)HEADER_FIXED - 4u  // magic+version+size = 8 bytes
	                  + SIGNATURE_FIELD + PADDING_SIZE + image_size;
	// HEADER_FIXED-4 = 8 bytes for magic+version+size
	sign_len = 8u + (size_t)SIGNATURE_FIELD + (size_t)PADDING_SIZE + image_size;

	if (out_buf_capacity < sign_len)
		return 0;

	uint8_t* write_ptr = out_buf;

	write_le16(write_ptr, (uint16_t)IMAGE_MAGIC);
	write_ptr += 2;
	write_le16(write_ptr, image_version);
	write_ptr += 2;
	write_le32(write_ptr, (uint32_t)image_size);
	write_ptr += 4;
...
int build_signed_image(const uint8_t* image_data, size_t image_size, const sig_algo_t* algo,
                       ...
                       uint8_t** raw_sig_out, size_t* raw_sig_len_out, uint8_t** sign_region_out,
                       size_t* sign_region_len_out)
{
	int ret             = -1;
	...
	if (make_sign_region(image_data, image_size, image_version, sign_reg, sign_len) != sign_len)
	{
	...
	if (raw_sig_len_out)
		*raw_sig_len_out = sig_written;
	if (sign_region_out)
	{
		*sign_region_out = sign_reg;
		sign_reg         = NULL;  // transfer ownership
	}
	if (sign_region_len_out)
		*sign_region_len_out = sign_len;
