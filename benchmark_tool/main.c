...
static void join_path(char* out_buf, size_t out_len, const char* dir, const char* filename)
{
...
static result_t run_one(const sig_algo_t* algo, const uint8_t* image_data, size_t image_size,
                        const char* keys_dir, const char* out_dir, uint16_t image_version,
                        int force_keygen)
{
	result_t r;
	memset(&r, 0, sizeof(r));
	...
	snprintf(priv_path, sizeof(priv_path), "%s%c%s_private%s", keys_dir, PATH_SEP, safe,
	         algo->key_extension);
	snprintf(pub_path, sizeof(pub_path), "%s%c%s_public%s", keys_dir, PATH_SEP, safe,
	         algo->key_extension);
	...
	double start_time = now_sec();
	int ret  = build_signed_image(image_data, image_size, algo, ctx, out_path, image_version,
	                              &raw_sig, &raw_sig_len, &sign_region, &sign_reg_len);
...
#define W_VERIFY 12
	...
	printf("\n");
	for (int column_index = 0; column_index < total_w; ++column_index)
		putchar('=');
	printf("\n  BENCHMARK SUMMARY\n");
	for (int column_index = 0; column_index < total_w; ++column_index)
		putchar('=');
	...
	printf("%-*s%*s%*s%*s  %s\n", W_ALGO, "Algorithm", W_KEYGEN, "KeyGen(ms)", W_SIGN, "Sign(ms)",
	       W_VERIFY, "Verify(ms)", "Status");
	...
	for (int row_index = 0; row_index < result_count; ++row_index)
	{
		...
		if (result->keygen_s < 0)
			...
			snprintf(keygen_s, sizeof(keygen_s), "%*.1f", W_KEYGEN, result->keygen_s * 1000.0);

		if (result->sign_s < 0)
			snprintf(sign_s, sizeof(sign_s), "%*s", W_SIGN, "n/a");
		else
			snprintf(sign_s, sizeof(sign_s), "%*.1f", W_SIGN, result->sign_s * 1000.0);

		if (result->verify_s < 0)
			snprintf(verify_s, sizeof(verify_s), "%*s", W_VERIFY, "n/a");
		else
			snprintf(verify_s, sizeof(verify_s), "%*.1f", W_VERIFY, result->verify_s * 1000.0);

		printf("%-*s%s%s%s  %s\n", W_ALGO, result->name, keygen_s, sign_s, verify_s,
		       result->status_msg);
	}
...
#define RW_VFY 12
	...
	for (int i = 0; i < total_w; ++i)
		putchar('=');
	printf("\n");
	printf("%-*s%-*s%*s%*s  %s\n", RW_ALGO, "Algorithm", RW_SIZE, "Size", RW_SIGN, "Sign(ms)",
	       RW_VFY, "Verify(ms)", "Status");
	for (int i = 0; i < total_w; ++i)
		putchar('-');
	printf("\n");
	...
	for (int i = 0; i < n; ++i)
	{
		...
		printf("%-*s%-*s%s%s  %s\n", RW_ALGO, rr[i].algo_name, RW_SIZE, rr[i].size_label, sign_buf,
		       vfy_buf, (rr[i].status == 0) ? "OK" : "ERROR");
	}
	for (int i = 0; i < total_w; ++i)
		putchar('=');
	printf("\n");
...
static int run_random_benchmarks(int image_version, const char* keys_dir, const char* out_dir,
                                 rand_result_t* rr_out, int* n_out)
{
	*n_out = 0;
	...
	for (int entry_index = 0; entry_index < s_registry_n; ++entry_index)
	{
		...
		printf("\n--- %s ", algo->name);
		for (int d = (int)strlen(algo->name); d < 52; ++d)
			putchar('-');
		printf("\n");
		...
		for (int s = 0; s < N_RAND_SIZES; ++s)
		{
			...
			double t0  = now_sec();
			int ret    = build_signed_image(rdata, sz, algo, ctx, out_path, (uint16_t)image_version,
			                                &raw_sig, &raw_sig_len, &sign_region, &sign_reg_len);
			...
			printf("  [%s] Sign: %.1f ms  |  Verify: %.1f ms  |  %s\n", label, rr->sign_s * 1000.0,
			       rr->verify_s * 1000.0, (ok == 1) ? "PASS" : "FAIL");
			rr->status = (ok == 1) ? 0 : 2;
...
int main(int argc, char** argv)
{
	...
	printf("\n");
	for (int banner_index = 0; banner_index < 67; ++banner_index)
		putchar('=');
	...
	for (int entry_index = 0; entry_index < s_registry_n; ++entry_index)
	{
		...
		results[n_results++] = run_one(s_registry[entry_index].algo, image_data, image_size,
		                               keys_dir, image_dir, (uint16_t)image_version, force_keygen);
	}
	...
	int n_rand = 0;
	run_random_benchmarks(image_version, keys_dir, image_dir, rand_results, &n_rand);
