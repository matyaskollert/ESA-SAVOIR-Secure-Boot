...
static int lms_write_cb(const byte* priv, word32 priv_len, void* context)
{
...
static int lms_generate_keys(sig_ctx_t* ctx, const char* priv_path, const char* pub_path)
{
	lms_ctx_t* context = (lms_ctx_t*)ctx;
	int result_code;
...
static int lms_load_keys(sig_ctx_t* ctx, const char* priv_path, const char* pub_path)
{
	lms_ctx_t* context = (lms_ctx_t*)ctx;
	int result_code;
	...
	if (pub_path)
	{
		...
		if (io_read_file(pub_path, &file_buffer, &buffer_length) != 0)
			return -1;
		result_code = wc_LmsKey_ImportPubRaw(&context->key, file_buffer, (word32)buffer_length);
		free(file_buffer);
		if (result_code != 0)
		{
...
static int lms_sign(sig_ctx_t* ctx, const uint8_t* msg, size_t msg_len, uint8_t* sig_out,
                    size_t* sig_written)
{
	lms_ctx_t* context = (lms_ctx_t*)ctx;
	...
	int result_code =
	    wc_LmsKey_Sign(&context->key, sig_out, &signature_len, (byte*)msg, (word32)msg_len);
	if (result_code != 0)
	{
...
static int lms_verify(sig_ctx_t* ctx, const uint8_t* msg, size_t msg_len, const uint8_t* sig,
                      size_t sig_len)
{
	lms_ctx_t* context = (lms_ctx_t*)ctx;

	int result_code =
	    wc_LmsKey_Verify(&context->key, (byte*)sig, (word32)sig_len, (byte*)msg, (word32)msg_len);
	if (result_code != 0)
	{
